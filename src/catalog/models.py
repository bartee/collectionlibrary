...
class TimestampedModel(models.Model):
    ...
    """
    created_at = models.DateTimeField(verbose_name="Creation date", auto_created=True)
...
class Collection(PrivateShareableModel, TimestampedModel):
    ...
    shared_with = models.ManyToManyField(to=settings.AUTH_USER_MODEL, related_name='co_owners')
...
class EntityResource(models.Model):
    ...
    RESOURCE_TYPES = (('image', 'Entity Image'), ('url','Entity Link'))

    entity = models.ForeignKey(to=Entity, on_delete=models.CASCADE, blank=True, default=None)

    type = models.CharField(max_length=25, choices=RESOURCE_TYPES)
