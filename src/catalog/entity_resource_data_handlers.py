...
class LinkEntityResourceTypeHandler(BaseEntityResourceTypeHandler):
    def validate(self, data):
        ...
        if not url:
            raise forms.ValidationError(
                _("No URL was entered while the type was set to link.")
            )
    ...
    def display_content(self, instance, style="full", *args, **kwargs):
        ...
        return '<a href="{url}" target="_blank">{text}</a>'.format(
            url=url, text=instance.description
        )
...
class ImageEntityResourceTypeHandler(BaseEntityResourceTypeHandler):
    ...
    def validate(self, data):
        ...
        if not image:
            raise forms.ValidationError(
                _("No Image was entered while the type was set to image.")
            )
...
class TextEntityResourceTypeHandler(BaseEntityResourceTypeHandler):
    ...
    def validate(self, data):
        ...
        if not text:
            raise forms.ValidationError(
                _("No text was entered, while the type was set to text")
            )
...
ENTITY_TYPE_ICONS = {"url": "link", "image": "image", "text": "short_text"}
...
class EntityResourceTypeFactory(object):
    ...
    def get_handler_by_type(type):
        ...
        :return:
        """
        handler = EntityResourceTypeFactory.ENTITY_TYPE_HANDLERS.get(
            type, BaseEntityResourceTypeHandler
        )
        return handler()
